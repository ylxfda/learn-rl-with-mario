...
class PPOTrainer:
    ...
    def __init__(self, args):
        ...
        Config.PPO_EPOCHS = args.ppo_epochs if args.ppo_epochs is not None else Config.PPO_EPOCHS
        Config.CLIP_EPSILON = args.clip_epsilon if args.clip_epsilon is not None else Config.CLIP_EPSILON
        Config.STEPS_PER_UPDATE = args.steps_per_update if args.steps_per_update is not None else Config.STEPS_PER_UPDATE
        Config.NUM_ENVS = args.num_envs if args.num_envs is not None else Config.NUM_ENVS
        Config.MAX_EPISODES = args.max_episodes if args.max_episodes is not None else Config.MAX_EPISODES
        Config.MAX_STEPS = args.max_steps if args.max_steps is not None else Config.MAX_STEPS
        Config.SAVE_FREQ = args.save_freq if args.save_freq is not None else Config.SAVE_FREQ
        Config.LOG_FREQ = args.log_freq if args.log_freq is not None else Config.LOG_FREQ
        Config.RENDER_ENV = args.render_env if args.render_env is not None else None
    ...
    def _init_environment(self):
        ...
        self.envs = create_parallel_mario_envs(
            ...
            render_env_id=Config.RENDER_ENV
        )
    ...
    def train_step(self):
        ...
        # 4) Bump total steps
        self.ppo.total_steps += Config.STEPS_PER_UPDATE * Config.NUM_ENVS
    ...
    def should_stop_training(self):
        ...
        """
        # Max steps
        if self.ppo.total_steps >= Config.MAX_STEPS:
            return True, f"Reached max steps {Config.MAX_STEPS:,}"
        
        # Max episodes
        if self.ppo.total_episodes >= Config.MAX_EPISODES:
    ...
    def train(self):
        ...
        try:
            while True:
                ...
                if update_count % Config.LOG_FREQ == 0:
                ...
                if update_count % Config.SAVE_FREQ == 0:
                    # Evaluate
